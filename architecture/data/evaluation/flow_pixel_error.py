...
def flow_calc_error(est_flow=None, gt_flow=None, lb=0.0, ub=400, sparse=False):
    """
    Args:
    ...
    # get valid mask
    # [B, 1, H, W]
    mask = torch.ones(gt_u.shape, dtype=torch.bool, device=gt_u.device)
    if sparse:
        mask = mask & (~(zero_mask(gt_u) & zero_mask(gt_v)))
    mask = mask & (~(torch.isnan(gt_u) | torch.isnan(gt_v)))

    rad = torch.sqrt(gt_u**2 + gt_v**2)
    mask = mask & (rad > lb) & (rad < ub)
